package vn.attendance.service.student.service.impl;
...
public class StudentServiceImpl implements StudentService {
    ...
    @Override
    public Page<Users> searchStudent1(String search, String curriculumName, String status, int page, int size) {
        Pageable pageable = PageRequest.of(page - 1, size);
    ...
    public AddStudentCurriculum addStudentCurriculum(AddStudentCurriculum request) throws AmsException {
        ...
        studentCurriculum.setCourseId(request.getCourseId());
    ...
    public EditStudentCurriculum updateStudentCurriculum(Integer id, EditStudentCurriculum request) throws AmsException {
        ...
        Optional<StudentCurriculum> oldStudentCurriculum = studentCurriculumRepository.findById(id);

        if (!oldStudentCurriculum.isPresent()) {
            throw new AmsException(MessageCode.CURRICULUM_NOT_FOUND);
        }

        if (oldStudentCurriculum.get().getCourseId() == request.getCourseId() &&
                oldStudentCurriculum.get().getStatus().equals(Constants.STATUS_TYPE.ACTIVE)) {
            request.setStatus("FAILED");
            request.setErrorMess(MessageCode.STUDENT_ALREADY_HAVE_CURRICULUM.toString());
            return request;
        } else {
            oldStudentCurriculum.get().setStatus(Constants.STATUS_TYPE.DELETED);
            StudentCurriculum newStudentCurriculum = new StudentCurriculum();

            newStudentCurriculum.setStudentId(oldStudentCurriculum.get().getStudentId());
            newStudentCurriculum.setCourseId(request.getCourseId());
            ...
            studentCurriculumRepository.save(oldStudentCurriculum.get());
