...
import vn.attendance.lib.structure.NETDEV_VIRTUALCAMERA_STATE_INFO;
...
public class DeviceServiceImpl implements DeviceService{
    ...
    @Override
    public HashMap<String, CameraInfoDto> GetListCamera() {
        ...
        int cameraCount = serverInstance.getDeviceInfo().byChanNum;  // 通道号个数
    ...
    @Override
    public boolean AttachCameraState() {
        ...
        stuIn.cbCamera = CameraStateCallBack.getInstance(socketController, cameraRepository);  // 注册回调
    ...
    public static class CameraStateCallBack implements NetSDKLib.fCameraStateCallBack {
        ...
        private CameraStateCallBack(SocketController socketController, CameraRepository cameraRepository) {
            this.socketController = socketController;
            this.cameraRepository = cameraRepository;
        }

        public static CameraStateCallBack getInstance(SocketController socketController, CameraRepository cameraRepository) {
            if (instance == null) {
                instance = new CameraStateCallBack(socketController, cameraRepository);
            }
            return instance;
        }
        ...
        public void invoke(NetSDKLib.LLong lLoginID, NetSDKLib.LLong lAttachHandle, Pointer pBuf, int nBufLen, Pointer dwUser) {
            ...
            if(camera!=null){
                camera.setStatus(ParseCameraStatus(cameraState.emConnectState));
                var cameraStatus = new CameraStateRes();
                cameraStatus.setChannel(cameraState.nChannel);
                cameraStatus.setStatus(ParseCameraStatus(cameraState.emConnectState));
                socketController.sendCameraStateMessage(cameraStatus);
                System.out.println("Channel:" + cameraStatus.getChannel() + " StatusCode:" + cameraState.emConnectState + "Status: "+ParseCameraStatus(cameraState.emConnectState));
    ...
    public void SynchronizeCamera() {
        ...
        var cameraDtos = GetListCamera();
        for (Camera camera: cameras
             ) {
            if(cameraDtos.containsKey(camera.getIpTcip())&& cameraDtos.get(camera.getIpTcip()).getPort() == Integer.parseInt(camera.getPort())){
    ...
    private String getStateCamera(Integer channelId) {
        ...
        if(bRet) {
            ...
            for (int i = 0; i < stOut.nValidNum; ++i) {
                System.out.printf("Channel%d:%s ", arrCameraStatus[i].nChannel, ParseCameraStatus(arrCameraStatus[i].emConnectionState));
