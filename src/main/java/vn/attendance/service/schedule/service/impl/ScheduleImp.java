...
public class ScheduleImp implements ScheduleService {
    ...
    private void init() {
        ...
        roomIndexMap = roomRepository.findAllRooms().stream()
                .collect(Collectors.toMap(Room::getRoomName, Room::getId));

        timeSlotIndexMap = timeSlotRepository.findAllTimeSlots().stream()
                .collect(Collectors.toMap(TimeSlot::getDescription, TimeSlot::getId));

        classIndexMap = classRoomRepository.findAllClassRoom().stream()
                .collect(Collectors.toMap(ClassRoom::getClassName, ClassRoom::getId));

        subIndexMap = subjectRepository.findAllSubject().stream()
                .collect(Collectors.toMap(Subject::getCode, Subject::getId));

        teacherIndexMap = teacherRepository.findAllRoleTeacher().stream()
                .collect(Collectors.toMap(Users::getUsername, Users::getId));
    ...
    @Override
    public ScheduleDto getScheduleById(Integer id) throws AmsException {
    ...
    public String[][] findScheduleByUserId(LocalDate date) throws Exception {
        ...
        for (int j = 1; j <= col; j++) {
            schedule[0][j] = timeSlotList.get(j - 1).getDescription() +
                    "\n(" + timeSlotList.get(j - 1).getStartTime().format(formatter) +
                    " - " + timeSlotList.get(j - 1).getEndTime().format(formatter) + ")";
            colIndex.put(timeSlotList.get(j - 1).getDescription(), j);
        }
        ...
        for (TimeSlot timeSlot : timeSlotList) {
            timeSlotIndexMap.put(timeSlot.getDescription(), timeSlot.getId());
        }
    ...
    public String[][] findScheduleByStudentId(LocalDate from, LocalDate to) throws Exception {
        ...
        Map<String, Integer> rowIndex = new HashMap<>();
        for (int j = 1; j <= row; j++) {
            schedule[j][0] = timeSlotList.get(j - 1).getDescription() +
                    "\n(" + timeSlotList.get(j - 1).getStartTime().format(formatter) +
                    " - " + timeSlotList.get(j - 1).getEndTime().format(formatter) + ")";
            rowIndex.put(timeSlotList.get(j - 1).getDescription(), j);
        }
    ...
    public List<AddScheduleRequest> importSchedule(List<AddScheduleRequest> stringList, String semesterName) throws AmsException {
        ...
        Map<String, String> timeSlotIndexMap = timeSlotList.stream().collect(Collectors.toMap(TimeSlot::getDescription, TimeSlot::getDescription));
        ...
        for (AddScheduleRequest request : stringList) {
            ...
            for (TimeSlot slot : timeSlotList) {
                if (timeOfDay.equals("AM")) if (slot.getStartTime().isBefore(LocalTime.of(12, 0))) {
                    timeSlotIndexMap.put(slot.getDescription(), slot.getDescription());
                    slots.add(slot);
                }
            ...
            for (int i = 1; i < characters.length; i++) {
                for (LocalDate localDate : days) {
                    scheduleList = scheduleRepository.findAllSchedules(localDate, semester.getId());
                    if (timeSlotIndexMap.containsKey(slots.get(i - 1).getDescription()) &&
                            localDate.getDayOfWeek().equals(getDayOfWeekName(Character.getNumericValue(characters[i])))) {

                        request.setTimeSlot(timeSlotIndexMap.get(slots.get(i - 1).getDescription()));
                        request.setDate(localDate);
                        addSchedule(request, semesterName, localDate);
    ...
    public AddScheduleRequest addSchedule(AddScheduleRequest request, String semesterName, LocalDate date) throws AmsException {
        Users users = BaseUserDetailsService.USER.get();
        if (users == null) {
            throw new AmsException(MessageCode.USER_NOT_FOUND);
        }
...
//        scheduleList = scheduleRepository.findAllSchedules(date, semesterId);
    ...
    public Object checkExit(Map<Object, Integer> scheduleIndexMap, Object request, Integer timeSlotId,
        ...
        for (Map.Entry<Object, Integer> entry : scheduleIndexMap.entrySet()) {
            ...
            if (schedule.getTimeSlotId().equals(timeSlotId) && schedule.getRoomId().equals(roomId) && schedule.getClassId().equals(classId)
                ...
                setRequestStatus(request, "FAILED", MessageCode.SCHEDULE_FAILED_SUBJECT.getCode());
                return request;
            }
            if (schedule.getTimeSlotId().equals(timeSlotId) && schedule.getRoomId().equals(roomId) && !schedule.getClassId().equals(classId)
                    && schedule.getUserId().equals(userId) && schedule.getSubjectId().equals(subId)) {

                setRequestStatus(request, "FAILED", MessageCode.SCHEDULE_FAILED_CLASS.getCode());
                return request;
            }
            if (schedule.getTimeSlotId().equals(timeSlotId) && schedule.getRoomId().equals(roomId) && schedule.getClassId().equals(classId)
                    && !schedule.getUserId().equals(userId) && schedule.getSubjectId().equals(subId)) {

                setRequestStatus(request, "FAILED", MessageCode.SCHEDULE_FAILED_TEACHER.getCode());
                return request;
            }
            if (schedule.getTimeSlotId().equals(timeSlotId) && schedule.getRoomId().equals(roomId) && schedule.getClassId().equals(classId)
                    && schedule.getUserId().equals(userId) && schedule.getSubjectId().equals(subId)) {

                setRequestStatus(request, "FAILED", MessageCode.SCHEDULE_ALREADY_EXISTS.getCode());
                return request;
            }
            if (schedule.getTimeSlotId().equals(timeSlotId) && schedule.getRoomId().equals(roomId) && !schedule.getClassId().equals(classId)) {
                setRequestStatus(request, "FAILED", MessageCode.SCHEDULE_FAILED_CLASS_TIME.getCode());
                return request;
            }
        }
        return null;
    ...
    public void setRequestStatus(Object request, String status, String errorMess) {
        if (request instanceof AddScheduleRequest) {
            AddScheduleRequest addRequest = (AddScheduleRequest) request;
            addRequest.setStatus(status);
            addRequest.setErrorMess(errorMess);
        } else if (request instanceof EditScheduleRequest) {
            EditScheduleRequest editRequest = (EditScheduleRequest) request;
    ...
    public List<LocalDate> getDatesFromDayOfWeek(String input, LocalDate fromDate, LocalDate toDate) {
        ...
        LocalDate lastDayOfMonth = endYearMonth.atEndOfMonth();
    ...
    public EditScheduleRequest editSchedule(EditScheduleRequest request, LocalDate date) throws AmsException {
        ...
        Semester semester = semesterRepository.findSemesters(date);
        List<Schedule> scheduleList = scheduleRepository.findAllSchedules(date, semester.getId());
