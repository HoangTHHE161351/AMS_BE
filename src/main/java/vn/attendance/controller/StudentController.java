...
public class StudentController {
    ...
    public ApiResponse<Page<Users>> searchStudent(@RequestParam(required = false) String search,
                                                  @RequestParam(required = false) String status,
                                                  @RequestParam(required = false) String curriculumName,
                                                  @RequestParam(defaultValue = "1") int page,
                                                  @RequestParam(defaultValue = "10") int size) {
        return ApiResponse.okStatus(studentService.searchStudent1(search, curriculumName, status, page, size));
    }
    ...
    public ApiResponse<?> deleteStudentCurriculum(@RequestParam Integer id) throws AmsException {
        Optional<StudentCurriculum> deletedStudentCurriculum = studentService.deleteStudentCurriculum(id);
        if (deletedStudentCurriculum.isPresent())
            return ApiResponse.okStatus(deletedStudentCurriculum.get());
