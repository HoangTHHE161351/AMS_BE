...
public interface UsersRepository extends JpaRepository<Users, Integer> {
    ...
    @Query("select u from Users u where u.id = :id and u.status <> 'DELETED' and u.roleId <> 1")
    Optional<Users> findUsersById(Integer id);

    @Query("select u from Users u join Role r on u.roleId = r.id" +
            " WHERE r.roleName = :roleName" +
            " AND u.status <> 'DELETED'")
    List<Users> findUserByRole(@Param(value = "roleName")String roleName);

    @Query( value = "SELECT u.id as id, r.role_name as roleName, CONCAT(u.last_name, ' ', u.first_name) as fullName, " +
            " u.avata as avatar" +
            "FROM users u " +
            "JOIN roles r ON u.role_id = r.id and r.status = 'ACTIVE' " +
            ...
            "OR u.phone LIKE CONCAT('%', :search, '%')) " +
            "ORDER BY u.created_at DESC", nativeQuery = true)
    Page<IFaceDto> GetListFace(String search, String roleName, String status, Integer gender, Pageable pageable);
